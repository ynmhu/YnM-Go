...
func NewMovieDeletionPlugin(bot *YnMIrC.Client, adminPlugin *owner.YnmAdminPlugin, movieDBPath string) *MovieDeletionPlugin {
...
func (p *MovieDeletionPlugin) HandleMessage(msg YnMIrC.Message) string {
    ...
    // Csak a parancsokra reagáljunk, először ellenőrizzük a prefixet
    text := strings.TrimSpace(msg.Text)
    if !(strings.ToLower(text) == strings.ToLower(prefix+"del") || strings.HasPrefix(strings.ToLower(text), strings.ToLower(prefix+"del "))) {
        return ""
    }

    // Ellenőrizzük az admin szintet
...
func (p *MovieDeletionPlugin) OnTick() []YnMIrC.Message {
	return nil
}